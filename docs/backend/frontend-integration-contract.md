...
Deployers must configure `DATABASE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_STORAGE_BUCKET`, and `UPLOAD_TICKET_SECRET` in server-safe environment configuration. The validated Vercel deployment uses a Supabase Transaction Pooler URL for database connectivity, while migrations may use a managed session connection; Drizzle remains the application-schema path and the private bucket is accessed only through server services. See [integration-readiness-review.md](integration-readiness-review.md) for the live integration evidence and server-mediated RLS decision.
