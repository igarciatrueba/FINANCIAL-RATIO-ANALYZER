...
const { uploadAnnualReportAction } = vi.hoisted(() => ({ uploadAnnualReportAction: vi.fn() }));

vi.mock("@/app/workspace/actions", () => ({ uploadAnnualReportAction }));
...
describe("annual-report authenticated upload", () => {
  it("sends a selected PDF to the authenticated private extraction flow and returns its review draft", async () => {
    ...
    uploadAnnualReportAction.mockResolvedValue({ draft });
    ...
    await waitFor(() => expect(uploadAnnualReportAction).toHaveBeenCalledTimes(1));
