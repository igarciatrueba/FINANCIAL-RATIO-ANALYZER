...
export function CreateCompanyForm() {
  ...
  return <form action={action} className="grid gap-3 rounded-lg border border-border bg-surface/50 p-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_7rem_auto] sm:items-end"><label className="grid gap-1 text-caption font-semibold text-neutral-300">Company name<input className="min-h-10 rounded-md border border-border bg-background px-3 text-small text-white" name="name" required /></label><label className="grid gap-1 text-caption font-semibold text-neutral-300">Industry<input className="min-h-10 rounded-md border border-border bg-background px-3 text-small text-white" name="industry" required /></label><label className="grid gap-1 text-caption font-semibold text-neutral-300">Currency<select className="min-h-10 rounded-md border border-border bg-background px-3 text-small text-white" defaultValue="EUR" name="currency"><option>EUR</option><option>USD</option><option>GBP</option></select></label><Button disabled={pending} type="submit">{pending ? <LoaderCircle aria-hidden="true" className="h-4 w-4 animate-spin" /> : <Plus aria-hidden="true" className="h-4 w-4" />}Add company</Button><div className="sm:col-span-full"><ActionMessage state={state} /></div></form>;
}
