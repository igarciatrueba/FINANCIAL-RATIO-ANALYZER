...
export function RatioAnalysis({ viewModel }: { viewModel: RatioAnalysisViewModel }) {
  ...
  return <div className="premium-workspace premium-ambient grid min-w-0 gap-9 premium-enter">
    <RatioContext viewModel={viewModel} />
    <section aria-label="Ratio category navigation" className="data-rail py-4"><div className="flex flex-wrap items-center gap-x-6 gap-y-2 px-4">{viewModel.categories.map((item) => <button aria-pressed={item.id === category} className={`border-b-2 px-0 py-2 text-small font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary ${item.id === category ? "border-primary text-white" : "border-transparent text-neutral-500 hover:text-neutral-200"}`} key={item.id} onClick={() => { setCategory(item.id); setMetricId(viewModel.trend.metricsByCategory[item.id][0]?.metricId ?? viewModel.trend.defaultMetricId); }} type="button">{item.label} <span className="ml-1 font-mono text-caption">{item.count}</span></button>)}</div></section>
    <section aria-label="Selected financial ratio" className="crystal-surface premium-score-shift grid gap-7 rounded-lg p-5 md:p-7 xl:grid-cols-[minmax(16rem,0.75fr)_minmax(0,1.25fr)]" key={selected.metricId}>
    ...
    <section aria-label="Detailed ratio table" className="min-w-0"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="premium-kicker">Full catalogue</p><h2 className="mt-2 text-h2 font-semibold tracking-tight text-white">Explore all {group?.label} ratios</h2></div><p className="max-w-sm text-small text-neutral-400">{viewModel.availability} of {viewModel.configured} outputs are available. An unavailable result is never shown as zero.</p></div><div className="mt-6 max-w-full overflow-x-auto border-y border-border"><table className="financial-ratio-table min-w-[980px] border-collapse text-left text-caption"><thead className="sticky top-0 text-neutral-300"><tr>{["Ratio", "Current", "Previous", "Change", "Financial direction", "Availability", "Formula and interpretation"].map((heading) => <th className="border-b border-border px-3 py-3 font-semibold" key={heading}>{heading}</th>)}</tr></thead><tbody className="divide-y divide-border">{group?.rows.map((row) => <tr className="align-top transition-colors" key={row.label}><td className="px-3 py-4 font-semibold text-neutral-50">{row.label}</td><td className="px-3 py-4 font-mono tabular-nums">{row.currentValue.display}</td><td className="px-3 py-4 font-mono tabular-nums text-neutral-300">{row.previousValue.display}</td><td className="px-3 py-4 font-mono tabular-nums">{row.change.display}</td><td className="px-3 py-4 text-neutral-300">{viewModel.financialDirection(row.label.toLowerCase().replaceAll(" ", "-"), row.direction)}</td><td className="px-3 py-4 text-neutral-300">{row.availability}{row.unavailableReason ? <span className="block text-neutral-500">{row.unavailableReason}</span> : null}</td><td className="px-3 py-4"><details><summary className="cursor-pointer font-semibold">Formula and interpretation</summary><p className="font-technical mt-3 text-neutral-200">{row.formula}</p><p className="mt-2 text-neutral-300">{row.interpretation}</p><p className="mt-2 text-neutral-400">{row.description}</p></details></td></tr>)}</tbody></table></div></section>
  </div>;
}

function RatioContext({ viewModel }: { viewModel: RatioAnalysisViewModel }) { return <section className="analytical-surface rounded-lg p-5"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="premium-kicker">Analytical context</p><p className="mt-2 text-h3 font-semibold text-white">{viewModel.company.name}</p><p className="text-small text-neutral-400">{viewModel.company.industry}</p></div><div className="grid grid-cols-2 gap-x-7 gap-y-3 text-small sm:flex sm:gap-7"><Info label="Current period" value={String(viewModel.period.current)} /><Info label="Comparison" value={viewModel.period.previous ? String(viewModel.period.previous) : "Unavailable"} /><Info label="Currency" value={viewModel.company.currency} /><Info label="Coverage" value={`${viewModel.coverage.toFixed(0)}%`} /></div><div className="flex flex-wrap gap-2"><Action href="/analysis" icon={Gauge} label="Overview" /><Action href="/analysis/dupont" icon={Network} label="DuPont" /><Action href="/scenario" icon={SlidersHorizontal} label="Scenario" /><Action href="/methodology" icon={BookOpen} label="Methodology" /></div></div></section>; }
function Info({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) { return <div><p className="text-caption uppercase tracking-[0.08em] text-neutral-500">{label}</p><p className={`mt-1 text-small font-semibold text-neutral-100 ${mono ? "font-technical" : ""}`}>{value}</p></div>; }
