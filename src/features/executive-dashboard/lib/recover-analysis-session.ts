import { analyseFinancialStatements, parseFinancialAnalysisInput } from "@/domain";
import { ACTIVE_ANALYSIS_SCHEMA_VERSION } from "@/features/financial-input/persistence";
import { buildExecutiveDashboardViewModel } from "@/features/executive-dashboard/lib/build-dashboard-view-model";
import type { DashboardSessionRecoveryResult } from "@/features/executive-dashboard/types/dashboard.types";

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

export function recoverExecutiveDashboardSession(serialized: string | null): DashboardSessionRecoveryResult {
  if (!serialized) {
    return { status: "empty" };
  }

  let parsed: unknown;
  try {
    parsed = JSON.parse(serialized);
  } catch {
    return {
      status: "corrupt",
      message: "The active analysis session could not be read.",
    };
  }

  if (!isRecord(parsed) || parsed.schemaVersion !== ACTIVE_ANALYSIS_SCHEMA_VERSION || typeof parsed.savedAt !== "string") {
    return {
      status: "corrupt",
      message: "The active analysis session is not compatible with this dashboard.",
    };
  }

  const canonical = parseFinancialAnalysisInput(parsed.data);
  if (!canonical.success) {
    return {
      status: "invalid",
      message: "The active analysis session failed canonical validation.",
      issues: canonical.validation.issues.map((issue) => issue.message),
    };
  }

  try {
    const analysis = analyseFinancialStatements(canonical.data);
    return {
      status: "ready",
      viewModel: buildExecutiveDashboardViewModel(analysis, canonical.data),
    };
  } catch {
    return {
      status: "analysis-error",
      message: "The accepted financial statements could not be analysed safely.",
    };
  }
}
